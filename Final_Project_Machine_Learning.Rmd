...
Algorithm|Accuracy
---------|--------
Gradient Boosted Machine|as.numeric(cm_GBM$overall[1])
Random Forests|as.numeric(cm_RF$overall[1])
Decision Trees|as.numeric(cm_DT$overall[1])

#Predicting in the Testing Data

The goal is to use the prediction model to predict 20 different test cases. the algorithm to use is the GBM because it has better precision. The following code makes the prediction for the 20 test cases.

```{r}
predict_final<-predict(fitGBM,newdata = testing_complete)
```
